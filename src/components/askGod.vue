...
const go = () => {
  ...
  const valueList = value.split("").map((i: any) => Number(i));
  const check = !!valueList.filter((i: number) => isNaN(i)).length;
...
const calcResult = (valueList: any[]) => {
  const lenCount = valueList.length === 1 ? 0 : valueList.length - 1;
  const sum = valueList.reduce((sumValue: any, cur: any) => (sumValue += cur));
  console.log(sum);
  ...
  <div class="box">
    ...
    <div class="input-wrapper">
      ...
      <el-popover
        ...
        :width="400"
        trigger="hover"
        content="想预测什么事，心念一动，立刻取数，取数可以是心中所想，也可以是看到的比如车牌号、门牌号、电话号码、时间等，要第一反应想到或看到的数。比如想问朋友在不在家，心念一动，恰巧看到手机屏幕上显示的时间是15:30分，就取1530进行测算；"
      >
        <template #reference>
          <el-input
            placeholder="请输入活数"
      ...
      <el-button type="primary" style="margin-left: 16px" @click="go"
        >起课</el-button
      >
    </div>
